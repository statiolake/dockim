...
async fn run_server(port: u16, mut shutdown_rx: oneshot::Receiver<()>, allowed_addresses: Arc<Vec<String>>) -> Result<()> {
    ...
    // Server started log is handled in neovim.rs
                ...
                match result {
                    Ok((stream, addr)) => {
                        ...
                        if !allowed_addresses.contains(&client_ip) {
                            log!("Rejected": "connection from {} (not allowed)", addr);
                            continue;
                        }
